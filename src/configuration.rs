...
pub struct Constructs {
    ...
    ///
    /// Gates both `[[…]]` links and `![[…]]` embeds — they share one grammar,
    /// mdast shape, and serializer, so they enable together.
    ///
    /// ```markdown
    /// > | a [[Page#Heading|Alias]] and ![[image.png]] b
    ///       ^^^^^^^^^^^^^^^^^^^^^^     ^^^^^^^^^^^^^^
    /// ```
    ///
    /// Defaults to off, and `serde(default)` keeps older serialized
    /// `Constructs` (written before this field existed) deserializable.
